...
def create_lost_report(report: LostReportCreate, db=Depends(get_db)):
    return report_store.create_lost_report(db, report.user_id, report.item_id, report.title, 
...
def edit_lost_report(report_id: int, report: LostReportCreate, db=Depends(get_db)):
    return report_store.edit_lost_report(db, report_id, report.title, report.description, 
